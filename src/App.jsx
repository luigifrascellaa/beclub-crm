...
const JUNG = [
  { key:"blu",    label:"BLU",    sub:"Metodo e professionalità",  desc:"Analitico, preciso, orientato al processo.",    bg:"linear-gradient(135deg,#3b4fd4,#6366f1)", border:"#6366f1", glow:"#6366f155" },
  ...
  { key:"verde",  label:"VERDE",  sub:"Disposizione all'aiutare",  desc:"Empatico, paziente, affidabile.",               bg:"linear-gradient(135deg,#047857,#10b981)", border:"#10b981", glow:"#10b98155" },
];
...
function AuthScreen({ onAuth }) {
  ...
  async function submit() {
    ...
    try {
      const res = mode === "login" ? await sbSignIn(email, pass) : await sbSignUp(email, pass);
      if (res && res.access_token) {
        onAuth({ token: res.access_token, userId: res.user.id, email: res.user.email });
      } else if (mode === "signup") {
        setErr("Registrazione ok! Ora accedi con le stesse credenziali.");
        setMode("login");
      } else {
        setErr("Credenziali non valide");
      }
    } catch(e) {
      setErr(e.message || "Errore di connessione");
    }
    setLoading(false);
  }
      ...
      <div className="pop" style={{ width:"100%", maxWidth:400, background:"#080f1f", border:"1px solid #1e3a5f", borderRadius:20, padding:"2.2rem", boxShadow:"0 20px 70px #000000aa" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:28 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"#3b5478", marginTop:2 }}>Pipeline · Profilazione · Statistiche</div>
          </div>
        </div>
        ...
        {err && (
          <div style={{ background:"#ef444415", border:"1px solid #ef444435", borderRadius:9, padding:"9px 13px", fontSize:12, color:"#f87171", marginBottom:14, lineHeight:1.5 }}>{err}</div>
        )}
...
export default function App() {
  ...
  async function handleLogout() {
    try { await sbSignOut(auth.token); } catch(e) { /* ignore */ }
  ...
  async function updateProfilo(id, profilazione) {
    ...
    try {
      ...
      setData(d => d.map(x => x.id === id ? upd : x));
      setSel(upd);
  ...
  if (!ready) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:"#060b18", flexDirection:"column", gap:12 }}>
      ...
      <span style={{ fontSize:13, color:"#3b5478" }}>Caricamento dati...</span>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", height:"100vh", width:"100vw", overflow:"hidden", background:"#060b18" }}>
      {toast && (
        <div style={{ position:"fixed", bottom:24, right:24, zIndex:9999, background:toast.color, color:"#fff", padding:"12px 22px", borderRadius:12, fontWeight:700, fontSize:13, boxShadow:"0 8px 30px #00000060", animation:"fadeIn .25s ease" }}>
          {toast.msg}
        </div>
      )}
      {saving && (
        <div style={{ position:"fixed", top:14, right:14, zIndex:9998, background:"#0d1b33", border:"1px solid #1e3a5f", borderRadius:9, padding:"7px 14px", fontSize:12, color:"#60a5fa", display:"flex", alignItems:"center", gap:7 }}>
          <span className="spinner" />Salvataggio...
        </div>
      )}

      <Sidebar view={view} setView={setView} data={data} urgenti={urgenti} onAdd={openAdd} onExport={onExport} auth={auth} onLogout={handleLogout} />
...
// ─── SIDEBAR ──────────────────────────────────────────────────────────────────
function Sidebar({ view, setView, data, urgenti, onAdd, onExport, auth, onLogout }) {
  const navs = [
      ...
      <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:"1.5rem", gap:12, flexWrap:"wrap" }}>
        <div>
          <div style={{ fontSize:11, fontWeight:700, color:"#2563eb", textTransform:"uppercase", letterSpacing:1.4, marginBottom:4 }}>
            Ciclo {dashCiclo}{dashCiclo === CICLO_CORRENTE ? " · in corso" : ""}
          </div>
          ...
          <div style={{ display:"flex", gap:10, marginTop:16, paddingTop:14, borderTop:"1px dashed #11203a" }}>
            {[{ f:"FOLLOW_UP", n:cdFU.length }, { f:"NON_INT", n:cdNI.length }].map(({ f, n }) => (
              <div key={f} style={{ flex:1, background:FASE_CLR[f] + "12", border:"1px solid " + FASE_CLR[f] + "28", borderRadius:10, padding:"10px 12px", display:"flex", alignItems:"center", gap:8 }}>
                ...
                <div>
                  <div style={{ fontWeight:900, fontSize:18, color:FASE_CLR[f] }}>{n}</div>
                  <div style={{ fontSize:10, color:"#3b5478", marginTop:1 }}>{FASE_LABEL[f]}</div>
                </div>
              </div>
            ))}
          </div>
...
function Statistiche({ data }) {
  ...
  const lineData = cicli.map(c => {
    const row = { ciclo: "C" + c };
    FASI_FUNNEL.forEach(f => { row[f] = data.filter(p => reachedInCiclo(p, f, c)).length; });
    return row;
  });

  const barData = FASI_FUNNEL.map(f => {
    const count = barCiclo === "ALL"
      ? data.filter(p => reachedEver(p, f)).length
      : data.filter(p => reachedInCiclo(p, f, Number(barCiclo))).length;
    return { fase: FASE_LABEL[f], key: f, count, fill: FASE_CLR[f] };
  });

  const tableRows = [...cicli].sort((a,b) => b-a).map(c => {
    const r = { c };
    FASI_FUNNEL.forEach(f => { r[f] = data.filter(p => reachedInCiclo(p, f, c)).length; });
    r.conv = r.INVITO > 0 ? Math.round(r.SUB / r.INVITO * 100) : r.FUP1 > 0 ? Math.round(r.SUB / r.FUP1 * 100) : 0;
    return r;
  });
  ...
  if (!data.length) return (
    <div style={{ padding:"2rem 2.2rem" }}>
      <h1 style={{ fontWeight:900, fontSize:26, color:"#eff6ff", marginBottom:8 }}>Statistiche</h1>
      <div style={{ textAlign:"center", padding:"5rem", color:"#1e3a5f" }}>
        <div style={{ fontSize:44, marginBottom:12 }}>◪</div>
        <p style={{ fontSize:14 }}>Aggiungi prospect per vedere le statistiche</p>
      </div>
    </div>
  );
    ...
    <div style={{ padding:"2rem 2.2rem", maxWidth:1280, margin:"0 auto" }}>
      ...
      <div style={{ background:"#080f1f", border:"1px solid #11203a", borderRadius:14, padding:"1.4rem", marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
          <div>
            <div style={{ fontSize:13, fontWeight:800, color:"#eff6ff" }}>📈 Andamento nei cicli</div>
            <div style={{ fontSize:11, color:"#3b5478", marginTop:2 }}>Quanti ne fai per ciclo</div>
          </div>
          <select value={linePhase} onChange={e => setLinePhase(e.target.value)} style={{ width:"auto", minWidth:160 }}>
            <option value="ALL">Tutte le fasi</option>
            {FASI_FUNNEL.map(f => <option key={f} value={f}>{FASE_LABEL[f]}</option>)}
          </select>
        </div>
        <div style={{ height:300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={lineData} margin={{ top:5, right:10, left:-15, bottom:5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#11203a" />
              <XAxis dataKey="ciclo" stroke="#3b5478" fontSize={12} />
              <YAxis stroke="#3b5478" fontSize={12} allowDecimals={false} />
              <Tooltip contentStyle={ts} cursor={{ stroke:"#1e3a5f" }} />
              {linePhase === "ALL"
                ? FASI_FUNNEL.map(f => <Line key={f} type="monotone" dataKey={f} name={FASE_LABEL[f]} stroke={FASE_CLR[f]} strokeWidth={2} dot={{ r:3 }} />)
                : <Line type="monotone" dataKey={linePhase} name={FASE_LABEL[linePhase]} stroke={FASE_CLR[linePhase]} strokeWidth={3} dot={{ r:4 }} activeDot={{ r:6 }} />
              }
              {linePhase === "ALL" && <Legend wrapperStyle={{ fontSize:11 }} />}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ background:"#080f1f", border:"1px solid #11203a", borderRadius:14, padding:"1.4rem", marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
          <div>
            <div style={{ fontSize:13, fontWeight:800, color:"#eff6ff" }}>📊 Conversione del percorso</div>
            <div style={{ fontSize:11, color:"#3b5478", marginTop:2 }}>Quanti passano da una fase all&apos;altra</div>
          </div>
          <select value={barCiclo} onChange={e => setBarCiclo(e.target.value)} style={{ width:"auto", minWidth:160 }}>
            <option value="ALL">Tutti i cicli</option>
            {[...cicli].sort((a,b) => b-a).map(c => <option key={c} value={c}>Ciclo {c}</option>)}
          </select>
        </div>
        <div style={{ height:300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} margin={{ top:5, right:10, left:-15, bottom:5 }}>
              ...
              <XAxis dataKey="fase" stroke="#3b5478" fontSize={12} />
              <YAxis stroke="#3b5478" fontSize={12} allowDecimals={false} />
              <Tooltip contentStyle={ts} cursor={{ fill:"#0d1b3360" }} />
              <Bar dataKey="count" name="Raggiunti" radius={[6,6,0,0]}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
          {barData.slice(0, -1).map((b, i) => {
            ...
            const rate = b.count > 0 ? Math.round(next.count / b.count * 100) : 0;
            return (
              <div key={i} style={{ flex:"1 1 120px", background:"#0a1426", border:"1px solid #11203a", borderRadius:9, padding:"9px 11px" }}>
                <div style={{ fontSize:10, color:"#3b5478", fontWeight:600 }}>{b.fase} → {next.fase}</div>
                <div style={{ fontSize:18, fontWeight:900, color:next.fill, marginTop:2 }}>{rate}%</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ background:"#080f1f", border:"1px solid #11203a", borderRadius:14, overflow:"hidden" }}>
        <div style={{ padding:"1.1rem 1.4rem", borderBottom:"1px solid #11203a" }}>
          <div style={{ fontSize:13, fontWeight:800, color:"#eff6ff" }}>📋 Cicli a confronto</div>
        </div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:600 }}>
            <thead>
              <tr style={{ borderBottom:"1px solid #11203a" }}>
                <th style={{ textAlign:"left", color:"#3b5478", fontWeight:700, fontSize:10, textTransform:"uppercase", letterSpacing:.8, padding:"11px 16px" }}>Ciclo</th>
                {FASI_FUNNEL.map(f => <th key={f} style={{ textAlign:"center", color:FASE_CLR[f], fontWeight:700, fontSize:10, textTransform:"uppercase", padding:"11px 10px" }}>{FASE_LABEL[f]}</th>)}
                <th style={{ textAlign:"center", color:"#3b5478", fontWeight:700, fontSize:10, textTransform:"uppercase", padding:"11px 16px" }}>Conv%</th>
              </tr>
            </thead>
            <tbody>
              {tableRows.map(r => (
                <tr key={r.c} className="hrow" style={{ borderBottom:"1px solid #0d1b3355" }}>
                  <td style={{ padding:"11px 16px" }}>
                    <span style={{ background:r.c === CICLO_CORRENTE ? "#2563eb22" : "#11203a", color:r.c === CICLO_CORRENTE ? "#60a5fa" : "#5278a8", borderRadius:6, padding:"3px 9px", fontSize:11, fontWeight:700 }}>C{r.c}</span>
                  </td>
                  {FASI_FUNNEL.map(f => <td key={f} style={{ textAlign:"center", padding:"11px 10px", fontWeight:700, fontSize:13, color:r[f] > 0 ? "#eff6ff" : "#2a4060" }}>{r[f]}</td>)}
                  <td style={{ textAlign:"center", padding:"11px 16px", fontWeight:800, fontSize:13, color:r.conv >= 20 ? "#10b981" : r.conv >= 10 ? "#0ea5e9" : "#f59e0b" }}>{r.conv}%</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div style={{ padding:"2rem 2.2rem", maxWidth:1280, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
        ...
        <select value={fFonte} onChange={e => setFFonte(e.target.value)} style={{ flex:1, minWidth:120 }}>
          <option value="">Tutte le fonti</option>
          {FONTI.map(f => <option key={f}>{f}</option>)}
        </select>
        <select value={fCiclo} onChange={e => setFCiclo(e.target.value)} style={{ flex:1, minWidth:140 }}>
      ...
      {prospects.length === 0 ? (
        <div style={{ textAlign:"center", padding:"4rem", color:"#1e3a5f" }}>
          <div style={{ fontSize:44, marginBottom:12 }}>◆</div>
          <p style={{ fontSize:14, marginBottom:14 }}>Nessun prospect trovato</p>
          <button onClick={onAdd} style={{ padding:"9px 20px", fontSize:13, fontWeight:800, background:"linear-gradient(135deg,#2563eb,#0ea5e9)", color:"#fff", border:"none", borderRadius:10, cursor:"pointer" }}>Aggiungi il primo</button>
        </div>
      ) : (
        <div style={{ background:"#080f1f", border:"1px solid #11203a", borderRadius:14, overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ borderBottom:"1px solid #11203a" }}>
                {["Prospect","Ciclo","Conosciuto","Fonte","Fase","Profilo","Pers.",""].map(h => (
                  <th key={h} style={{ textAlign:"left", color:"#3b5478", fontWeight:700, fontSize:10, textTransform:"uppercase", letterSpacing:.8, padding:"12px 16px", whiteSpace:"nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={p.id} className="hrow" onClick={() => onOpen(p)} style={{ cursor:"pointer", borderBottom:"1px solid #0d1b3355" }}>
                    <td style={{ padding:"12px 16px" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <Av n={p.nome} c={p.cognome} color={FASE_CLR[p.fase]} />
                        <span style={{ color:"#eff6ff", fontWeight:700, fontSize:13 }}>{p.nome} {p.cognome}</span>
                      </div>
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      {c ? <span style={{ background:c === CICLO_CORRENTE ? "#2563eb22" : "#11203a", color:c === CICLO_CORRENTE ? "#60a5fa" : "#3b5478", borderRadius:6, padding:"2px 8px", fontSize:11, fontWeight:700 }}>C{c}</span> : <span style={{ color:"#2a4060" }}>\u2014</span>}
                    </td>
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      <span style={{ display:"inline-flex", alignItems:"center", borderRadius:6, padding:"3px 9px", fontSize:11, fontWeight:700, color:"#fff", background:FASE_CLR[p.fase], boxShadow:"0 0 8px " + FASE_CLR[p.fase] + "35" }}>{FASE_LABEL[p.fase]}</span>
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      {badge.compilati === 0
                        ? <span style={{ color:"#2a4060", fontSize:11 }}>\u2014</span>
                        : <span style={{ display:"inline-flex", alignItems:"center", gap:4, borderRadius:6, padding:"3px 9px", fontSize:11, fontWeight:800, color:bc, background:bc + "18", border:"1px solid " + bc + "30" }}>🎯 {badge.positivi}/{PROFILO_TOTAL}</span>
                      }
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      {jung
                        ? <span title={jung.sub} style={{ display:"inline-flex", alignItems:"center", gap:6, borderRadius:6, padding:"3px 9px", fontSize:11, fontWeight:800, color:jung.border, background:jung.border + "18", border:"1px solid " + jung.border + "35" }}>
                            <span style={{ width:8, height:8, borderRadius:"50%", background:jung.border, flexShrink:0, boxShadow:"0 0 6px " + jung.border }} />
                            {jung.label}
                          </span>
                        : <span style={{ color:"#2a4060", fontSize:11 }}>\u2014</span>
                      }
  ...
  return (
    <div style={{ background:"#080f1f", border:"1px solid #1e3a5f", borderRadius:16, padding:"1.6rem", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 70px #000000aa" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
        ...
        <div style={{ gridColumn:"1/-1" }}><label style={lbl}>Città</label><input value={form.citta||""} onChange={e => set("citta", e.target.value)} placeholder="es. Milano" /></div>
        <div>
          <label style={lbl}>Fonte</label>
          <select value={form.fonte||"Instagram"} onChange={e => set("fonte", e.target.value)}>
            {FONTI.map(f => <option key={f} value={f}>{FONTE_ICO[f]} {f}</option>)}
          </select>
        </div>
        <div>
          <label style={lbl}>Fase</label>
          <select value={form.fase||"INVITO"} onChange={e => set("fase", e.target.value)}>
            <optgroup label="Funnel">{FASI_FUNNEL.map(f => <option key={f} value={f}>{FASE_LABEL[f]}</option>)}</optgroup>
            <optgroup label="Speciali">{FASI_SPECIALI.map(f => <option key={f} value={f}>{FASE_LABEL[f]}</option>)}</optgroup>
          </select>
        </div>
        ...
        <div style={{ gridColumn:"1/-1" }}>
          <label style={lbl}>Ciclo <span style={{ color:"#3b5478", fontWeight:500, textTransform:"none", marginLeft:6, fontSize:11 }}>(auto dalla data)</span></label>
          <select value={cicloCalc} onChange={e => onCicloChange(Number(e.target.value))}>
            {CICLO_NUMS.map(c => <option key={c} value={c}>Ciclo {c} — {cicloLabel(c)}</option>)}
          </select>
        </div>
      ...
      <div style={{ marginBottom:18 }}>
        ...
        <textarea value={form.note||""} onChange={e => set("note", e.target.value)} style={{ height:76, resize:"vertical" }} placeholder="Dove l'ho conosciuto, contesto..." />
      </div>
...
function ProfilazioneTab({ p, onUpdateProfilo }) {
  ...
  function toggle(section, key) {
    ...
    const next = nextToggle(current);
    onUpdateProfilo({ pleasures: { ...pr.pleasures }, forza: { ...pr.forza }, [section]: { ...(pr[section]||{}), [key]: next } });
  }

  function selectJung(key) {
    onUpdateProfilo({ pleasures: { ...pr.pleasures }, forza: { ...pr.forza }, jung: pr.jung === key ? null : key });
  }

  function ToggleGroup({ title, fields, section, icon }) {
    return (
      <div style={{ marginBottom:18 }}>
        <div style={{ fontSize:10, fontWeight:800, color:"#3b5478", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10, display:"flex", alignItems:"center", gap:6 }}>
              ...
              <div key={f.key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#0a1426", borderRadius:9, padding:"9px 12px", border:"1px solid " + (val != null ? clr + "40" : "#11203a") }}>
                ...
                <div style={{ display:"flex", gap:5 }}>
                  {TV.filter(v => v !== null).map(v => {
                    ...
                    const vc = TC[v];
                    return (
                      <button key={v} className="togbtn" onClick={() => toggle(section, f.key)}
  ...
  const badge = profiloBadge(p);
  const pct   = Math.round(badge.positivi / PROFILO_TOTAL * 100);
  const bc    = pct >= 60 ? "#10b981" : pct >= 30 ? "#0ea5e9" : "#f59e0b";
    ...
    <div>
      <div style={{ background:"#0a1426", borderRadius:10, padding:"12px 14px", marginBottom:16, border:"1px solid #11203a" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          <span style={{ fontSize:11, fontWeight:700, color:"#3b5478", textTransform:"uppercase", letterSpacing:.8 }}>Score profilazione</span>
          <span style={{ fontWeight:900, fontSize:16, color:bc }}>🎯 {badge.positivi}/{PROFILO_TOTAL}</span>
        </div>
        <div style={{ height:6, background:"#0d1b33", borderRadius:99, overflow:"hidden" }}>
          <div style={{ height:"100%", width:pct + "%", background:"linear-gradient(90deg," + bc + "88," + bc + ")", borderRadius:99, transition:"width .4s ease", boxShadow:"0 0 8px " + bc + "50" }} />
        </div>
        <div style={{ display:"flex", justifyContent:"space-between", marginTop:5 }}>
          <span style={{ fontSize:10, color:"#3b5478" }}>{badge.compilati}/{PROFILO_TOTAL} compilati</span>
          <span style={{ fontSize:10, color:bc, fontWeight:700 }}>{pct}% positivi</span>
        </div>
      </div>
      ...
      <div style={{ marginBottom:4 }}>
        <div style={{ fontSize:10, fontWeight:800, color:"#3b5478", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10, display:"flex", alignItems:"center", gap:6 }}>
          <span>🎨</span>Personalita — Colori Jung
        </div>
          ...
          {JUNG.map(j => {
            ...
            return (
              <button key={j.key} onClick={() => selectJung(j.key)}
                style={{ background:active ? j.bg : "#0a1426", border:"2px solid " + (active ? j.border : "#1e3a5f"), borderRadius:12, padding:"14px 14px 12px", cursor:"pointer", textAlign:"left", transition:"all .2s", boxShadow:active ? "0 0 18px " + j.glow : "none", position:"relative", overflow:"hidden" }}>
                {active && <div style={{ position:"absolute", top:8, right:10, width:18, height:18, borderRadius:"50%", background:"#ffffff33", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:900, color:"#fff" }}>✓</div>}
                <div style={{ fontWeight:900, fontSize:14, color:active ? "#fff" : j.border, marginBottom:3 }}>{j.label}</div>
                <div style={{ fontSize:10, fontWeight:700, color:active ? "rgba(255,255,255,.85)" : "#5278a8", marginBottom:5 }}>{j.sub}</div>
                <div style={{ fontSize:10, color:active ? "rgba(255,255,255,.65)" : "#3b5478", lineHeight:1.45 }}>{j.desc}</div>
              </button>
            );
          })}
        </div>
        {jd && (
          <div style={{ background:jd.border + "15", border:"1px solid " + jd.border + "35", borderRadius:10, padding:"10px 13px", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:10, height:10, borderRadius:"50%", background:jd.border, flexShrink:0, boxShadow:"0 0 8px " + jd.border }} />
            <div>
              <span style={{ fontSize:11, fontWeight:800, color:jd.border }}>{jd.label}</span>
              <span style={{ fontSize:11, color:"#5278a8", marginLeft:6 }}>· {jd.sub}</span>
            </div>
          </div>
        )}
        {!sj && (
          <div style={{ background:"#0a1426", borderRadius:9, padding:"9px 12px", border:"1px dashed #1e3a5f", textAlign:"center" }}>
            <span style={{ fontSize:11, color:"#2a4060" }}>Nessun colore selezionato</span>
          </div>
        )}
      </div>

      <div style={{ background:"#0a1426", borderRadius:9, padding:"10px 12px", border:"1px solid #11203a", marginTop:12 }}>
        <div style={{ fontSize:10, color:"#2a4060", fontStyle:"italic", lineHeight:1.5 }}>
          Le persone non comprano il prodotto, ma la trasformazione
        </div>
      </div>
    </div>
  );
...
function DetailModal({ p, onEdit, onAdvance, onFollowUp, onNonInt, onRiattiva, onClose, onUpdateProfilo }) {
  ...
  const clr        = FASE_CLR[p.fase];
  const ci         = FASI_FUNNEL.indexOf(p.fase);
  const isSpeciale = FASI_SPECIALI.includes(p.fase);
  const od         = isOver(p.followUp);
  const dt         = isToday(p.followUp);
  const ciclo      = cicloOfDate(p.conosciutoAt);
    ...
    <div style={{ background:"#080f1f", border:"1px solid #1e3a5f", borderRadius:16, padding:"1.6rem", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 70px #000000aa" }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:16, background:"#0a1426", padding:4, borderRadius:10, border:"1px solid #11203a" }}>
        {[{ id:"dettagli", label:"📋 Dettagli" }, { id:"profilazione", label:"🎯 Profilazione" }].map(t => (
          <button key={t.id} className="tabbtn" onClick={() => setActiveTab(t.id)}
            style={{ flex:1, background:activeTab===t.id?"#0d1b33":"transparent", color:activeTab===t.id?"#7dd3fc":"#5278a8", boxShadow:activeTab===t.id?"inset 0 0 0 1px #2563eb40":"none" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "dettagli" && (
        <>
            ...
            <div style={{ display:"flex", alignItems:"center", marginBottom:20, overflowX:"auto", paddingBottom:4 }}>
              {FASI_FUNNEL.map((f, i) => (
                <div key={f} style={{ display:"flex", alignItems:"center", flex:i < FASI_FUNNEL.length-1 ? 1 : "none" }}>
                  <div style={{ width:38, height:38, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", background:i <= ci ? FASE_CLR[f] : "#0d1b33", border:"2px solid " + (i === ci ? FASE_CLR[f] : i < ci ? FASE_CLR[f] + "66" : "#1e3a5f"), color:i <= ci ? "#fff" : "#3b5478", fontSize:7.5, fontWeight:900, boxShadow:i === ci ? "0 0 18px " + FASE_CLR[f] + "66" : "none", transition:"all .3s" }}>
                    {FASE_LABEL[f]}
                  </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:9, marginBottom:9 }}>
            {[
              { l:"Fase ora",          v:FASE_LABEL[p.fase], color:clr },
              { l:"Ciclo conoscenza",  v:ciclo ? "Ciclo " + ciclo : "\u2014", color:ciclo === CICLO_CORRENTE ? "#2563eb" : undefined },
              { l:"Conosciuto il",     v:fmt(p.conosciutoAt) },
              { l:"Follow-up",         v:p.followUp ? (od ? "⚠️ Scaduto · " : dt ? "📅 Oggi · " : "✅ ") + fmt(p.followUp) : "Non impostato", color:od ? "#f87171" : dt ? "#fbbf24" : undefined },
            ].map(({ l, v, color: col }) => (
              <div key={l} style={box}>
                <div style={lbl}>{l}</div>
                <div style={{ color:col || "#eff6ff", fontWeight:700, fontSize:13 }}>{v}</div>
              </div>
            ))}
          </div>
          ...
          {storico.length > 0 && (
            <div style={{ ...box, marginBottom:9 }}>
              <div style={lbl}>🛤️ Storico percorso</div>
              <div style={{ display:"flex", flexDirection:"column", gap:6, marginTop:8 }}>
                {storico.map((s, i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:9 }}>
                    <span style={{ width:8, height:8, borderRadius:99, background:FASE_CLR[s.fase], flexShrink:0, boxShadow:"0 0 6px " + FASE_CLR[s.fase] + "70" }} />
                    <span style={{ fontSize:12.5, fontWeight:700, color:"#eff6ff", minWidth:64 }}>{FASE_LABEL[s.fase]}</span>
                    <span style={{ fontSize:11, color:"#5278a8" }}>{fmt(s.data)}</span>
                    <span style={{ fontSize:10, color:"#3b5478", marginLeft:"auto" }}>Ciclo {cicloOfDate(s.data) || "\u2014"}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {p.note && (
            <div style={{ ...box, marginBottom:9 }}>
              ...
              <p style={{ color:"#94b5d8", lineHeight:1.6, fontSize:13, marginTop:4 }}>{p.note}</p>
            </div>
          )}

          <div style={{ display:"flex", gap:9, marginTop:16, flexWrap:"wrap" }}>
            {!isSpeciale && ci < FASI_FUNNEL.length-1 && (
              <button onClick={onAdvance} style={{ padding:"9px 16px", background:"linear-gradient(135deg," + FASE_CLR[FASI_FUNNEL[ci+1]] + "," + FASE_CLR[FASI_FUNNEL[ci+1]] + "bb)", color:"#fff", border:"none", borderRadius:9, cursor:"pointer", fontWeight:800, fontSize:12 }}>
                Avanza → {FASE_LABEL[FASI_FUNNEL[ci+1]]}
              </button>
            )}
            {isSpeciale && (
              <button onClick={onRiattiva} style={{ padding:"9px 16px", background:"linear-gradient(135deg,#2563eb,#0ea5e9)", color:"#fff", border:"none", borderRadius:9, cursor:"pointer", fontWeight:800, fontSize:12 }}>
                ↩ Riattiva nel Funnel
              </button>
            )}
          ...
          {!isSpeciale && (
            <div style={{ borderTop:"1px solid #0d1b33", marginTop:13, paddingTop:13, display:"flex", gap:9, flexWrap:"wrap" }}>
              <div style={{ fontSize:10, color:"#2a4060", width:"100%", fontWeight:700, textTransform:"uppercase", letterSpacing:.8, marginBottom:2 }}>Stato speciale</div>
              {p.fase !== "FOLLOW_UP" && <button onClick={onFollowUp} style={{ padding:"8px 13px", background:"#f59e0b16", color:"#fbbf24", border:"1px solid #f59e0b38", borderRadius:9, cursor:"pointer", fontWeight:700, fontSize:12 }}>🔥 Follow Up caldo</button>}
              {p.fase !== "NON_INT"   && <button onClick={onNonInt}   style={{ padding:"8px 13px", background:"#ef444414", color:"#f87171", border:"1px solid #ef444436", borderRadius:9, cursor:"pointer", fontWeight:700, fontSize:12 }}>❌ Non interessato</button>}
            </div>
          )}
        </>
      )}
